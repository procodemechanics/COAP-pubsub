...
+        }
+#endif
+	printf("UPDATE: uri=%s ct=%u rt=%u val=%s expire=%u current=%x next=%x child=%x\n",
...
@@ -534,7 +577,6 @@ CoapPDU::Code post_create_handler(Resource* resource, const char* in, char* &pay
...
@@ -548,6 +590,9 @@ CoapPDU::Code put_publish_handler(Resource* resource, CoapPDU* pdu) {
...
+
     while (num_options-- > 0) {
         if (options[num_options].optionNumber == CoapPDU::COAP_OPTION_CONTENT_FORMAT) {
             ct_exists = true;
@@ -563,6 +608,16 @@ CoapPDU::Code put_publish_handler(Resource* resource, CoapPDU* pdu) {
...
+        }
     ...
     if (!ct_exists) {
@@ -575,6 +630,11 @@ CoapPDU::Code put_publish_handler(Resource* resource, CoapPDU* pdu) {
...
@@ -594,8 +654,8 @@ void remove_all_resources(Resource* resource, bool is_head, Resource* parent, Re
...
@@ -785,7 +845,6 @@ int handle_request(char *uri_buffer, CoapPDU *recvPDU, int sockfd, struct sockad
...
-
     ssize_t sent = sendto(
         sockfd,
         response->getPDUPointer(),
@@ -868,26 +927,28 @@ int main(int argc, char **argv) {
